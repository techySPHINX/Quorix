...
class IPBlocklist:
    ...
    async def add_ip(self, ip: str, duration: int = 3600) -> None:
...
class RequestValidator:
    ...
    def validate_content_type(
        request: Request, allowed_types: Optional[List[str]] = None
    ) -> bool:
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_security_event(
        self, event_type: str, request: Request, details: str
    ) -> None:
    ...
    async def _track_attack_frequency(self, ip: str) -> None:
        ...
        if new_count >= self.attack_threshold:
            ...
            logger.warning(
                f"Security event: auto_block for IP {ip} - auto-blocked due to {new_count} suspicious requests"
            )
    ...
    async def dispatch(self, request: Request, call_next: Any) -> Response:
        ...
        if self.ip_blocklist.is_blocked(client_ip):
            await self._log_security_event(
                "blocked_ip", request, f"Blocked IP {client_ip} attempted access"
            )
            raise HTTPException(
        ...
        if not self.request_validator.validate_content_length(request):
            await self._log_security_event(
                "request_too_large", request, "Request exceeds maximum size"
            )
            raise HTTPException(
        ...
        if not self.request_validator.validate_content_type(request):
            await self._log_security_event(
                "invalid_content_type", request, "Invalid content type"
            )
            raise HTTPException(
        ...
        if not self.request_validator.validate_headers(request):
            await self._log_security_event(
                "invalid_headers", request, "Invalid or suspicious headers"
            )
            raise HTTPException(
        ...
        # 3. Threat detection
        is_suspicious, threat_details = self.threat_detector.is_suspicious_request(
            request
        )
        if is_suspicious:
        ...
        try:
            ...
            await self._log_security_event(
                "request_error", request, f"Request processing error: {str(e)}"
            )
            raise
...
class CSRFMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: Any, exempt_paths: Optional[List[str]] = None) -> None:
        super().__init__(app)
