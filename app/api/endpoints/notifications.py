...
async def mark_all_notifications_read(
    ...
    marked_count = await crud.notification.mark_all_read(
        db=db,
        user_id=current_user.id
    )
...
async def delete_notification(
    ...
    await crud.notification.delete_notification(
        db=db,
        notification_id=notification_id
    )
...
async def get_notification_stats(
    ...
    stats = await crud.notification.get_user_stats(
        db=db,
        user_id=current_user.id
    )
...
async def send_comprehensive_notification(
    ...
    results = await notification_service.send_notification(
        ...
        user_email=target_user.email if send_email else None,
        user_name=target_user.full_name if send_email else None,
    )
